...
export default function PRDDocument({ prd: initialPrd, onSectionSave }: PRDDocumentProps) {
  // Keep local copy of PRD so edits reflect immediately without page refresh
  ...
  const handleSave = (section: string) => async (value: unknown) => {
    ...
    // Update local state immediately — fixes the refresh bug
  ...
  return (
    <article className="bg-white rounded-xl border border-gray-200 shadow-sm divide-y divide-gray-100">
      ...
      <div className="px-8">
        {/* 1. Problem Statement — plain text editing */}
        ...
        {/* 2. User Personas — structured form editing */}
        ...
        {/* 3. Jobs to Be Done — string array editing */}
        ...
        {/* 4. User Stories — structured editing */}
                ...
                {prd.user_stories!.map((s, i) => (
                  <div key={i} className="flex items-start gap-3 py-2 border-b border-gray-50 last:border-0">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium mt-0.5 ${
                      ...
                      : s.priority === 'should-have' ? 'bg-yellow-100 text-yellow-700'
        ...
        {/* 5. Acceptance Criteria — structured editing */}
        ...
        {/* 6. Edge Cases — structured editing */}
              ...
              <div className="space-y-3">
                {prd.edge_cases!.map((ec, i) => (
                  <div key={i} className="rounded-lg bg-amber-50 border border-amber-100 p-3">
                    ...
                    <p className="text-sm text-amber-700 mt-1">{ec.expected_behavior}</p>
                  </div>
                ))}
              </div>
            )}
        ...
        {/* 7. Out of Scope — string array editing */}
        ...
        {/* 8. Success Metrics — structured editing */}
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {prd.success_metrics!.map((m, i) => (
                  <div key={i} className="rounded-lg border border-gray-200 p-4">
                    ...
                    <p className="text-2xl font-bold text-indigo-600 my-1">{m.target}</p>
        ...
        {/* 9. Rollout Plan — read only for now (too complex to edit in form) */}
        {prd.rollout_plan &&
            ...
            <PRDSection
              ...
              renderContent={() => (
                <div className="grid gap-4 sm:grid-cols-3">
                  {[prd.rollout_plan!.phase_1, prd.rollout_plan!.phase_2, prd.rollout_plan!.phase_3].map((phase, i) => (
                    <div key={i} className="rounded-lg border border-gray-200 p-4">
                      <p className="text-xs font-semibold text-indigo-600 uppercase tracking-wide">Phase {i + 1}</p>
                      ...
                      <p className="text-xs text-green-700 bg-green-50 rounded p-2">{phase.success_criteria}</p>
                    </div>
                  ))}
                </div>
              )}
        ...
        {/* 10. Open Questions — structured editing */}
              ...
              <div className="space-y-3">
                {prd.open_questions!.map((q, i) => (
                  <div key={i} className="rounded-lg bg-blue-50 border border-blue-100 p-4">
                    ...
                    <p className="text-xs text-blue-700 mt-1 italic">Impact if unresolved: {q.impact}</p>
                  </div>
                ))}
              </div>
            )}
